...
func (hs *NqHandshake) writeTokenS(out []byte) (int, error) {
	...
	// F138: PSK validity check - must have applied own randomness before sending S
	if hs.pattern.HasPSK() && !hs.ownRandApplied {
...
// F57: Sets ownRandApplied (PSK contributes entropy).
func (hs *NqHandshake) processTokenPsk() error {
	...
	hs.ownRandApplied = true // F57: PSK contributes entropy
